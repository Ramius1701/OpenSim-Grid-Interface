...
$mysqli = db();
if (!$mysqli) {
    echo '<div class="alert alert-danger my-3">Database connection failed.</div>';
    include_once "include/" . FOOTER_FILE; exit;
}
...
function fetch_destinations($mysqli,$db,$like_lower,$q,$limit){
    ...
    $regCols =['regionName','regionname','simname','region'];
        $dwellCol=null; foreach(['dwell','popularity'] as $c){ if(in_array($c,$cols,true)){ $dwellCol=$c; break; } }
...
$render_dest=function($r){
    ...
    if(!empty($r['desc'])) echo '<div class="small mt-1">'.nl2br(h($r['desc'])).'</div>';
    echo '</div>';
    if(!empty($r['dwell'])) echo '<span class="badge text-bg-secondary align-self-start">Popularity: '.h($r['dwell']).'</span>';
    echo '</div>';
};
