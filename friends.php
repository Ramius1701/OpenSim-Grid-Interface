...
function getAllFriends($con, $userId) {
    ...
    $sql = "
        SELECT g.OtherID,
            ...
            CASE
                WHEN pr.UserID IS NOT NULL THEN 1
                WHEN gu.Online = 1 OR (gu.Login IS NOT NULL AND gu.Login > (UNIX_TIMESTAMP() - 300)) THEN 1
                ELSE 0
            END AS Online
        FROM (
        ...
        LEFT JOIN Presence pr ON pr.UserID = g.OtherID
        ORDER BY Online DESC, gu.Login DESC, ua.FirstName ASC, ua.LastName ASC
    ";
...
function getOnlineFriends($con, $userId) {
    ...
    $sql = "
        ...
        OR gu.Online = 1
        OR (gu.Login IS NOT NULL AND gu.Login > (UNIX_TIMESTAMP() - 300))
...
<div class="container-fluid mt-4 mb-4">
    <div class="row">
        ...
        <div class="col-md-3">
            ...
            <div class="card mt-3">
                <div class="card-header">
                    <h5><i class="bi bi-bar-chart"></i> <?php echo $isLoggedIn ? 'Friend statistics' : 'Network overview'; ?></h5>
                </div>
                <div class="card-body">
                    <?php if ($isLoggedIn): ?>
                    ...
                    <?php else: ?>
                        <div class="text-center">
                            <p class="text-muted">
                                <i class="bi bi-lock"></i> Login required to view friend statistics
                            </p>
                            <a href="login.php?next=<?php echo urlencode($_SERVER['REQUEST_URI']); ?>" class="btn btn-primary btn-sm">
                                            ...
                                            <div class="d-grid gap-2">
                                                <?php if ($isLoggedIn && isset($friendMap[$user['PrincipalID']])): ?>
                                                    <span class="btn btn-success btn-sm disabled">
                                                        <i class="bi bi-check"></i> Already friends
                                                    </span>
                                                <?php elseif (!$isLoggedIn): ?>
                                                    <a href="login.php?next=<?php echo urlencode($_SERVER['REQUEST_URI']); ?>" class="btn btn-primary btn-sm">
                                                        <i class="bi bi-box-arrow-in-right"></i> Log in to add friend
                                                    </a>
                                                <?php elseif ($user['PrincipalID'] == $currentUserId): ?>
                                                    <span class="btn btn-secondary btn-sm disabled">
                                                        <i class="bi bi-person"></i> That's you
                                                    </span>
                                                <?php else: ?>
                    ...
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <div>
                            <a href="friends.php?action=online" class="btn btn-success btn-sm">
                                <i class="bi bi-circle-fill"></i> Online (<?php echo (int)$stats['online']; ?>)
                            </a>
                            <a href="friends.php?action=requests" class="btn btn-warning btn-sm">
                                <i class="bi bi-clock"></i> Requests (<?php echo (int)$stats['pending']; ?>)
                            </a>
                        </div>
                        <?php endif; ?>
                    </div>
                    <div class="card-body">
                        <?php if ($isLoggedIn): ?>
                            ...
                            <div class="text-center py-5">
                                ...
                                <a href="friends.php?action=search" class="btn btn-primary">
                                    <i class="bi bi-person-plus"></i> Find new friends
                                </a>
                            </div>
                            <?php endif; ?>
                        ...
                        <?php else: ?>
                            ...
                            <div class="text-center py-5">
                                ...
                                <a href="friends.php?action=search" class="btn btn-primary">
                                    <i class="bi bi-search"></i> Browse Users
                                </a>
                            </div>
                        <?php endif; ?>
                    </div>
